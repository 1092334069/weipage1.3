		...
		<ul class="tree-list">
			<li v-for="item in pluginList">
				...
				<div class="tree-child" v-if="item.pluginList && item.pluginList.length">
					...
					<plugin-tree-list v-if="showPluginId == item.pluginId" :plugin-list="item.pluginList" :select-plugin-id="selectPluginId" @select-plugin="selectPlugin"></plugin-tree-list>
				</div>
			</li>
		</ul>
	</div>
